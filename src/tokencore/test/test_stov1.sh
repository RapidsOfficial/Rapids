...
$SRCDIR/tokencore-cli --regtest token_sendissuancefixed $ADDR 1 1 0 "Z_TestCat" "Z_TestSubCat" "Z_IndivisTestProperty" "Z_TestURL" "Z_TestData" 100 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
...
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[1]} 3 5 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 10%% = 10 SP#3\n" ${ADDRESS[2]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[2]} 3 10 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 15%% = 15 SP#3\n" ${ADDRESS[3]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[3]} 3 15 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 20%% = 20 SP#3\n" ${ADDRESS[4]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[4]} 3 20 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 25%% = 25 SP#3\n" ${ADDRESS[5]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[5]} 3 25 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 25%% = 25 SP#3\n" ${ADDRESS[6]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[6]} 3 25 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "\nTesting a cross property (v1) STO, distributing 1000.00 SPT #4 to holders of SPT #3\n"
printf "   * Executing the transaction\n"
TXID=$($SRCDIR/tokencore-cli --regtest token_sendsto $ADDR 4 1000 "" 3)
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
...
TXID=$($SRCDIR/tokencore-cli --regtest token_sendactivation $ADDR 10 $(($BLOCKS + 8)) 999)
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
...
FEATUREID=$($SRCDIR/tokencore-cli --regtest token_getactivations | grep -A 10 completed | grep featureid | cut -c27-28)
if [ $FEATUREID == "10" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
...
printf "     # Checking the fee cache now has 0.00006000 fee cached for TOKEN... "
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[1]} 4 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[2]} 4 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[3]} 4 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[4]} 4 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[5]} 4 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[6]} 4 | grep balance | cut -d '"' -f4)
if [ $BALANCE == "250.00000000" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
