...
################################################################################################
## PLEASE DISABLE DEV TOKEN FOR THESE TESTS!!!                                                 ##
##                                                                                            ##
## The fee distribution tests require an exact amount of Token in each address.  Dev Token will ##
## skew these tests by increasing the amount of Token in the Exodus Address.                   ##
## To use this script in regtest mode, temporarily disable Dev Token and recompile by adding:  ##
##    return 0;                                                                               ##
## as the first line of the function calculate_and_update_devmsc in tokencore.cpp.  This line  ##
## must be removed and Token Core recompiled to use on mainnet.                                ##
################################################################################################
...
$SRCDIR/tokencore-cli --regtest token_sendissuancefixed $ADDR 2 1 0 "Z_TestCat" "Z_TestSubCat" "Z_IndivisTestProperty" "Z_TestURL" "Z_TestData" 10000000 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Creating a divisible test property in the test ecosystem\n"
$SRCDIR/tokencore-cli --regtest token_sendissuancefixed $ADDR 2 2 0 "Z_TestCat" "Z_TestSubCat" "Z_DivisTestProperty" "Z_TestURL" "Z_TestData" 10000000 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Generating addresses to use as fee recipients (OMN holders)\n"
ADDRESS=()
for i in {1..6}
do
   ADDRESS=("${ADDRESS[@]}" $($SRCDIR/tokencore-cli --regtest getnewaddress))
done
printf "   * Using a total of 1000 TOKEN\n"
printf "   * Seeding %s with 50.00 TOKEN\n" ${ADDRESS[1]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[1]} 1 50.0 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 100.00 TOKEN\n" ${ADDRESS[2]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[2]} 1 100.0 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 150.00 TOKEN\n" ${ADDRESS[3]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[3]} 1 150.0 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "   * Seeding %s with 200.00 TOKEN\n" ${ADDRESS[4]}
$SRCDIR/tokencore-cli --regtest token_send $ADDR ${ADDRESS[4]} 1 200.0 >$NUL
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
printf "\nActivating the fee system...\n"
printf "   * Sending the activation\n"
BLOCKS=$($SRCDIR/tokencore-cli --regtest getblockcount)
TXID=$($SRCDIR/tokencore-cli --regtest token_sendactivation $ADDR 9 $(($BLOCKS + 8)) 999)
$SRCDIR/tokencore-cli --regtest setgenerate true 1 >$NUL
...
FEESHARE=$($SRCDIR/tokencore-cli --regtest token_getfeeshare ${ADDRESS[1]} | grep feeshare | cut -d '"' -f4)
if [ $FEESHARE == "5.0000%" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
...
FEESHARE=$($SRCDIR/tokencore-cli --regtest token_getfeeshare ${ADDRESS[2]} | grep feeshare | cut -d '"' -f4)
if [ $FEESHARE == "10.0000%" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
...
FEESHARE=$($SRCDIR/tokencore-cli --regtest token_getfeeshare ${ADDRESS[3]} | grep feeshare | cut -d '"' -f4)
if [ $FEESHARE == "15.0000%" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
...
FEESHARE=$($SRCDIR/tokencore-cli --regtest token_getfeeshare ${ADDRESS[4]} | grep feeshare | cut -d '"' -f4)
if [ $FEESHARE == "20.0000%" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
...
FEESHARE=$($SRCDIR/tokencore-cli --regtest token_getfeeshare $ADDR | grep feeshare | cut -d '"' -f4)
if [ $FEESHARE == "50.0000%" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
...
FEESHARE=$($SRCDIR/tokencore-cli --regtest token_getfeeshare $ADDR 2 | grep feeshare | cut -d '"' -f4)
...
CACHEDFEE=$($SRCDIR/tokencore-cli --regtest token_getfeecache 1 | grep cachedfee | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance $ADDR 1 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance $ADDR 6 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[1]} 6 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[2]} 6 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[3]} 6 | grep balance | cut -d '"' -f4)
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance ${ADDRESS[4]} 6 | grep balance | cut -d '"' -f4)
...
$SRCDIR/tokencore-cli --regtest setgenerate true 51 >$NUL
...
CACHEDFEE=$($SRCDIR/tokencore-cli --regtest token_getfeecache 2147483651 | grep cachedfee | cut -d '"' -f4)
if [ $CACHEDFEE == "90" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
...
BALANCE=$($SRCDIR/tokencore-cli --regtest token_getbalance $ADDR 2147483651 | grep balance | cut -d '"' -f4)
if [ $BALANCE == "9999910" ]
  then
    printf "PASS\n"
    PASS=$((PASS+1))
