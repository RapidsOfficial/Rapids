...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeRapidsAmount") || reset)
        SoftSetArg("-anonymizerapidsamount", settings.value("nAnonymizeRapidsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rapids.cpp during shotdown to also remove the window positions
