...
#include "ui_zrpdcontroldialog.h"
...
std::set<std::string> ZRpdControlDialog::setSelectedMints;
std::set<CMintMeta> ZRpdControlDialog::setMints;

bool CZRpdControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRpdControlDialog::COLUMN_DENOMINATION || column == ZRpdControlDialog::COLUMN_VERSION || column == ZRpdControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRpdControlDialog::ZRpdControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRpdControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Label Style
    ui->labelZRpd->setProperty("cssClass", "text-main-purple");
    ui->labelZRpd_int->setProperty("cssClass", "text-main-purple");
    ...
    // click on checkbox
    connect(ui->treeWidget, &QTreeWidget::itemChanged, this, &ZRpdControlDialog::updateSelection);
    // push select/deselect all button
    connect(ui->pushButtonAll, &QPushButton::clicked, this, &ZRpdControlDialog::ButtonAllClicked);
}

ZRpdControlDialog::~ZRpdControlDialog()
{
    delete ui;
}

void ZRpdControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRpdControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRpdControlWidgetItem* itemDenom(new CZRpdControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRpdControlWidgetItem *itemMint = new CZRpdControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZRpdControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRpdControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRpd_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZRpdControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRpdControlDialog::GetSelectedMints()
{
...
void ZRpdControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
