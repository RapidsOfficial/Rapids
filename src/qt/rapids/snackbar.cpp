...
#include "qt/rapids/forms/ui_snackbar.h"
#include "qt/rapids/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(RapidsGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &RapidsGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
