...
#include "qt/rapids/forms/ui_dashboardwidget.h"
#include "qt/rapids/sendconfirmdialog.h"
#include "qt/rapids/txrow.h"
...
DashboardWidget::DashboardWidget(RapidsGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	if (window)
		connect(window, &RapidsGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, [this](){window->openFAQ();});
		connect(ui->btnHowTo, &QPushButton::clicked, window, [this](){window->openFAQ();});
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair RPD
