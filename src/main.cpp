            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
				...
				if (fAddressIndex) {
					...
					if (prevout.scriptPubKey.IsPayToScriptHash()) {
						...
						addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(2, uint160(hashBytes), hash, j), CAddressUnspentValue(prevout.nValue, prevout.scriptPubKey, undo.nHeight)));
						...
						// restore unspent index
						addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(1, uint160(hashBytes), hash, j), CAddressUnspentValue(prevout.nValue, prevout.scriptPubKey, undo.nHeight)));
