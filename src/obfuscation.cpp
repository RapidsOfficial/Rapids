...
{
    if (fMasterNode) LogPrintf("CObfuscationPool::Check() - entries count %lu\n", entries.size());
    //printf("CObfuscationPool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if (fMasterNode) {
        LogPrintf("CObfuscationPool::Check() - entries count %lu\n", entries.size());

        // If entries is full, then move on to the next phase
        if (state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions()) {
            LogPrintf("CObfuscationPool::Check() -- TRYING TRANSACTION \n");
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrintf("CObfuscationPool::Check() -- FINALIZE TRANSACTIONS\n");
    ...
    if (fMasterNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrintf("CObfuscationPool::Check() -- SIGNING\n");
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        LogPrintf("CObfuscationPool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();
...
void CObfuscationPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        // Reset
        LogPrintf("CObfuscationPool::Check() -- COMPLETED -- RESETTING\n");
        SetNull();
...
void CObfuscationPool::ChargeFees()
{
    ...
    //mostly offending?
    if (offences >= 3 - 1 && r > 33) return;

    //everyone is an offender? That's not right
    if (offences >= 3) return;
    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Rapids and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 RPD per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::CheckTimeout()
{
    ...
    if (!fMasterNode) {
        ...
        case POOL_STATUS_TRANSMISSION:
            LogPrintf("CObfuscationPool::CheckTimeout() -- Session complete -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_ERROR:
            LogPrintf("CObfuscationPool::CheckTimeout() -- Pool error -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_SUCCESS:
            LogPrintf("CObfuscationPool::CheckTimeout() -- Pool success -- Running Check()\n");
            Check();
            break;
        }
    }
    ...
    while (it != vecObfuscationQueue.end()) {
        if ((*it).IsExpired()) {
            LogPrintf("CObfuscationPool::CheckTimeout() : Removing expired queue entry - %d\n", c);
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE) {
        ...
        while (it2 != entries.end()) {
            if ((*it2).IsExpired()) {
                LogPrintf("CObfuscationPool::CheckTimeout() : Removing expired entry - %d\n", c);
        ...
        LogPrintf("CObfuscationPool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", OBFUSCATION_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if (state == POOL_STATUS_SIGNING && GetTimeMillis() - lastTimeChanged >= (OBFUSCATION_SIGNING_TIMEOUT * 1000) + addLagTime) {
        LogPrintf("CObfuscationPool::CheckTimeout() -- Session timed out (%ds) -- restting\n", OBFUSCATION_SIGNING_TIMEOUT);
        ChargeFees();
        UnlockCoins();
        SetNull();
...
{
    LogPrintf("CObfuscationPool::NewBlock \n");
