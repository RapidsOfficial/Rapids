...
std::list<CZerocoinMint> CzPIVTracker::ListZerocoinMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
...
void CzPIVTracker::SetPubcoinUsed(const uint256& hashPubcoin, const bool isUsed)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
std::list<CMintMeta> CzPIVTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        std::list<CZerocoinMint> listMintsDB = walletdb.ListMintedCoins();
