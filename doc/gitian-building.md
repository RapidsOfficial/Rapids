...
*Setup instructions for a gitian build of Rapids using a Debian VM or physical system.*
...
- [Building Rapids](#building-rapids)
...
Clone the git repositories for rapids and gitian and then checkout the rapids version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the rapids repository
...
To build Rapids (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the rapids repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/rapids/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/rapids/rapids
...
For example:
```bash
URL=https://github.com/konez2k/rapids.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit rapids=${COMMIT} --url rapids=${URL} ../rapids/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit rapids=${COMMIT} --url rapids=${URL} ../rapids/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit rapids=${COMMIT} --url rapids=${URL} ../rapids/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/rapids-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/rapids-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/rapids-build.assert
```
...
[rapids/gitian.sigs](https://github.com/rapids/gitian.sigs/) repository, or if that's not possible to create a pull
request.
