Rapids Core version *3.1.0* is now available from:  <https://github.com/RapidsOfficial/Rapids/releases>
...
Rapids Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zRPD Staking is here! zRPD staking will be activated on the morning of the 8th of May 2018. With the release of zRPD staking, there are effectively 2 versions of zRPD, zRPD minted on the 3.0.6 Rapids wallet or lower, and zRPD minted on Rapids wallet version or higher. New features in this release will require the use of zRPD v2, zRPD minted on this wallet release 3.1.0 or later. If you currently hold zRPD v1 and wish to take advantage of zRPD staking and deterministic zRPD, you will need to spend the zRPD v1 to yourself and remint zRPD v2.
...
To encourage the use of zRPD and increase the Rapids zerocoin anonymity set, the Rapids payment schedule has been changed to the following:
...
The visual layout of the Rapids Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zRPD minting feature. This is enabled by default and the enablezeromint=0 setting in the rapids.conf file will overwrite the GUI option.
...
### Summary
 ...
 The Rapids wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the rapids.conf file, found in the Rapids data directory.
...
Rapids Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The Rapids client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running rapidsd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the Rapids support group, which often times sees users say "I didn't spend that zRPD". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the Rapids masternodes, the monthly budget available to be utilised has been increased to 42,000 RPD / month. This RPD only has the opportunity to be raised once per month (paid to winning proposals) with any unused RPD not created by the blockchain.

### Budget Finalization Fee

The Rapids finalization fee for successful proposals has now been reduced, this fee is now 5 RPD down from 50 RPD. The total fee outlay for a successful proposal is now a total of 55 RPD.
...
Several critical security flaws in the zerocoin protocol and Rapids's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zRPD Control dialog in the QT wallet, a user is able to see zRPD marked as version 1 or 2.
...
Rapids's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which Rapids used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to Rapids using the bug bounty program. The modulus is now correctly imported and Rapids's accumulators have been changed to use the new proper modulus.
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault rapids-cli getinfo while loading block index (Mrs-X)
